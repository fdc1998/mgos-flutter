...
class TextRecognitionState extends ChangeNotifier {
  ...
  String get text => getSN(_data!.text);
  ...
  getSN(String data) {
    ...
    final sampleTextLines = splitter.convert(data.toString());
    RegExp exp = RegExp(r'(\w{12})');

    for (var i = 0; i < sampleTextLines.length; i++) {
      String str = sampleTextLines[i].replaceAll(' ', '');
      final match = exp.hasMatch(str);
      if (match == true) {
        if (str.contains('TBS'))
